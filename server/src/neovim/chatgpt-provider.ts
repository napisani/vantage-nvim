...
export class ChatGptProvider implements BackendProvider {
	...
	constructor(options: ChatGptProviderOptions = {}) {
		...
		this.apiKey = firstNonEmpty(options.apiKey, env.OPENAI_API_KEY);
...
function requireApiKey(value: string | undefined): string {
	...
	if (!apiKey) {
		throw new Error('ChatGPT provider requires config.provider.chatgpt.api_key or OPENAI_API_KEY.');
	}
