...
run-codex: dev-dirs compile
	$(DEV_ENV) LEARN_DEV_PROVIDER=codex LEARN_PROVIDER=codex LEARN_CODEX_MODEL="$(CODEX_MODEL)" LEARN_CODEX_COMMAND="$(CODEX_COMMAND)" LEARN_CODEX_TIMEOUT_MS="$(CODEX_TIMEOUT_MS)" $(NVIM) --noplugin -u "$(ROOT)/nvim/dev/init.lua" "$(FILE)"
